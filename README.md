...
- **Video playback** (`--features video`): a full player via a spawned `ffmpeg` binary (must be on PATH at runtime, no C linkage at build time): play/pause, seek bar plus J/L ±10s, volume, mute, and loop, with auto-hiding controls. Audio is the playback clock
...
cargo build --features video   # include video playback (ffmpeg on PATH at runtime)
